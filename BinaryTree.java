...
public class BinaryTree {
	 ...
	 */
	private void creatBinaryTree(int[] datas, LinkedList<Node> nodelist) {
		// 第一步先将数组中的每个值变成一个Node节点，然后添加进LinkedList集合中
		for (int nodeindex = 0; nodeindex < datas.length; nodeindex++) {
			Node node = new Node(datas[nodeindex]);
			nodelist.add(node);
		}
		// 给所有父节点设定子节点（这里不包含最后一个父节点 ）
	...
	/**
	 * 节点类,刚开始的时候先把数组的每个值变成节点值，并且另每个节点的左右子节点都为空 var 节点值 left 节点左子节点 right 右子节点
	 */
	class Node {
		int var;
		Node left;
